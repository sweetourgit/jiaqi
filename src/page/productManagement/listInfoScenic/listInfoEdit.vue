...
//      this.ruleForm.destinations = this.$route.params.destinations;
    ...
    methods: {
      ...
      dest(item) {
        this.dynamicTags.push({ "destination_id": item.id, "destination_name": item.value });
        this.ruleForm.destinations = "";
        this.inputVisible = false;
      },
      ...
      addsave(formName) {
        ...
        this.dynamicTags.forEach(function(v, k, arr) {
          object.push({
            ...
            "destination_name": arr[k]['destination_name']
          });
        });
        this.$refs[formName].validate((valid) => {
          ...
          if (valid) {
            var _this = this;
            this.$http.post(this.GLOBAL.serverSrcPhp + "/api/v1/product/product/upd", {
              "product_name": this.ruleForm.productName, //基本信息产品名称,
              "id": this.ruleForm.id,
              "destinations": object,
