<template>
  <div>
    <el-tabs v-model="activeName" @tab-click="handleClick" v-if="url_is">
      ...
      <el-tab-pane label="邮轮" name="4"></el-tab-pane>
    </el-tabs>
  ...
  export default {
    ...
    methods: {
      handleClick(tab, event) {
        if (tab.name == 1) {
          ...
          this.$router.push({ path: "/productList/productLiner" }); // 邮轮
        }
      }
    },
    watch: {
      $route: {
        handler: function(val, oldVal){
          if (val.path == "/productList/scenicOrTicketList"){ // 景区/票务
            ...
            this.activeName = "3"
          }else if(val.path == "/productList/productLiner"){ // 邮轮
            this.activeName = "4"
          }
        },
        // 深度观察监听
    ...
    created() {
      if (this.$route.path == "/productList/scenicOrTicketList"){ // 景区/票务
        ...
        this.activeName = "3"
      }else if(this.$route.path == "/productList/productLiner"){ // 邮轮
        this.activeName = "4"
      }
    },
  };
