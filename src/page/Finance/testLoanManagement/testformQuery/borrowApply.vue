 <!-- 无收入、预付款公用借款申请弹窗子组件 区别：预付款新增借款人数字段以及团期计划必填项需校验-->
<template>
  <div>
   <!-- 申请无收入借款弹窗 -->
   <el-dialog :title="paymentTypeTitle" :visible.sync="dialogFormVisible"  width="1100px" custom-class="city_list" @close="cancel">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
        <!-- 基本信息 -->
        <!-- 无收入不需检验的团期计划1 -->
        <el-form-item v-if="paymentType===1" prop="productType"  label="团期计划" style="float:left;">
            <el-select v-model="ruleForm.productType" placeholder="请选择业务" @change="productTypeChange">
                <el-option :label="item.label" :value="item.value" v-for="item in businessType" :key="item.value"> </el-option> 
            </el-select>
        </el-form-item>
         <el-form-item v-if="paymentType===1" prop="plan" label-width="0" style="float:left;margin-left:10px" >
            <el-input style="width:300px;" disabled v-model="ruleForm.plan" placeholder="请选择团期计划"></el-input>
            <el-input style="width:300px;" disabled v-model="ruleForm.plan_01" placeholder="通过输入团期计划,自动补充产品名称"></el-input>
            <el-button class="name_button" @click="IncomePlan(ruleForm.productType)">选择</el-button>
        </el-form-item>
         <!-- 无收入不需检验的团期计划1 end-->
        <!-- 预付款检验的团期计划2 -->
        <el-form-item v-if="paymentType===2" prop="productType"  label="团期计划" style="float:left;"  :rules="[{ required: true}]">
            <el-select v-model="ruleForm.productType" placeholder="请选择业务" @change="productTypeChange">
                <el-option :label="item.label" :value="item.value" v-for="item in businessType" :key="item.value"> </el-option> 
            </el-select>
        </el-form-item>
        <el-form-item v-if="paymentType===2" prop="plan" label-width="0" style="float:left;margin-left:10px" :rules="[{ required: true,message: '请选择团期计划'}]">
            <el-input style="width:300px;" disabled v-model="ruleForm.plan" placeholder="请选择团期计划"></el-input>
            <el-input style="width:300px;" disabled v-model="ruleForm.plan_01" placeholder="通过输入团期计划,自动补充产品名称"></el-input>
            <el-button class="name_button" @click="IncomePlan(ruleForm.productType)">选择</el-button>
        </el-form-item>
        ...
        <el-form-item label="供应商名称" prop="supplier" style="clear:both;">
            <el-autocomplete class="name_input"  :fetch-suggestions="querySearch3" v-model="ruleForm.supplier" placeholder="请输入供应商名称" :trigger-on-focus="false"  @select="departure"></el-autocomplete>
          <!--<el-input style="width:300px;" disabled v-model="ruleForm.supplier" placeholder="请通过模糊查询选择相应供应商"></el-input>-->
        </el-form-item>        
        <el-form-item label="借款类型" prop="planType" style="float:left;">
        ...
        <el-form-item label="借款人数"  prop="loanNumber" class="bright_b" label-width="120px" style="margin-left: 10px;float:left;" v-if="paymentType===2">
            <el-input type="number" v-model="ruleForm.loanNumber" class="lightspot inputWidth" placeholder="请输入人数"  style='margin-left: 10px;'></el-input>
        </el-form-item>
        <el-form-item label="借款金额" prop="planAmount" style="clear:both;">
          <el-input class="name_input" style="width:222px;" v-model="ruleForm.planAmount" placeholder="请输入借款金额"></el-input>
        </el-form-item>
        ...
        <el-form-item label="附件" style="clear:both;" prop="accessory">
          <el-upload class="upload-demo"  name="files" ref="upload"  multiple
            :action="this.upload_url"  :file-list="fileList"  :on-error="handleError"
      ...
      <div class="indialog">
        ...
        <div style="float:left">
          <div class="plan_indialog">
            ...
            <el-select v-model="planProductType" placeholder="请选择业务" style="float:left; margin-right:5px;" class="indialog_input">
                <el-option :label="item.label" :value="item.value" v-for="item in businessType" :key="item.value"> </el-option> 
            </el-select>
            <el-input class="indialog_input" v-model="plan_stage" clearable placeholder="请输入团期计划"></el-input>
          </div>
          <div class="plan_indialog">
            <span class="indialog_plan">出行日期</span>
            <el-date-picker class="indialog_input" style="width: 80%"  v-model="plan_data"
          ...
          <div style="float:left;margin-top:15px;margin-bottom: 15px;">
            <div class="plan_indialog">
                ...
                <el-input class="indialog_input" v-model="plan_name" clearable placeholder="请输入产品名称"></el-input>
            </div>
            <el-button class="indialog_button" @click="planSelect(planProductType)" type="primary">搜索</el-button>
    ...
    <el-dialog width="45%" title="选择账户" :visible.sync="dialogFormVisible_account" append-to-body>
      <div class="indialog">
        <el-table :data="tableAccount" @current-change="handleCurrentChangeAcount" ref="multipleTableBank" border  style="width: 100%; margin:30px 0 20px 0;"
          :header-cell-style="getRowClass"  @row-click="clickBank" @selection-change="changeFunBank" highlight-current-row>
...
import {common} from "../mixins/common.js";    // 导入混合对象
export default {
    data() {
    ...
    created(){
        console.log('shenq:::'+this.paymentType)
        this.themeList();//支付方式
        this.payment();// 借款类型
        this.getUserName();//获取全部用户列表（产品录入人使用）
         // 只有是出纳的时候才显示申请人检索(预付款)
    ...
    methods:{
        ...
        CloseNoIncomeShow(){
            this.$confirm("是否取消本次借款申请?", "提示", {
                ...
                this.$refs["ruleForm"].resetFields();
                this.clearPlan();                
                this.ruleForm.productType="1";//重置业务选择为1
                this.$emit('searchHandList', false);
                })
            .catch(() => {
                this.$message({
                ...
                this.$http.post(this.GLOBAL.serverSrc + "/finance/payment/api/insert",
                    {
                      object: {
                        ...
                        "paymentType": this.paymentType, // 借款类型 1无收入借款 2预付款
                        ...
                        "peopleCount": this.paymentType===2?this.ruleForm.loanNumber:0, //数量预付款
                        "payway": 0, //付款方式
                      }
                    })
                    .then(res => {
                      if(res.data.isSuccess == true){                       
                        //this.getList();//弹框调用这个页面的table不刷新，在引用弹框的页面调用
                        this.clearPlan();
                        this.$emit('searchHandList', false);
                        if(this.paymentType==2){//预付款
        ...
        clearPlan(){
            ...
            // 无收入借款中团期计划弹窗选择按钮（）
        IncomePlan(productTypeParams){
            this.saveType=productTypeParams;//存取来
            this.plan_stage = '';
            this.plan_name = '';
            this.plan_data = '';
            this.planList(productTypeParams);
            this.dialogFormVisible_plan = true;
        },
        // 查询无收入借款团期计划列表
        planList(productTypeParams){
            this.tablePlan=[];
            this.planProductType=productTypeParams;       
            if(productTypeParams==="1"){//参团的团期计划表       
            ...
            else if(productTypeParams==="4"){//游轮           
                this.$http.post(this.GLOBAL.serverSrcYL + "/linerapi/v1/sku/sku-plan/planlistpage",
                {
                    "pageIndex":this.ruleFormSeach.page ,
                    "pageSize": this.ruleFormSeach.limit,
                    "product_name": this.plan_name?this.plan_name:'',
                    "tour_no": this.plan_stage?this.plan_stage:'',
                    "start_at": this.plan_data[0] ? moment(this.plan_data[0]).format('YYYYMMDD') : 0, // 搜索用开始日期,
                    "end_at": this.plan_data[1] ? moment(this.plan_data[1]).format('YYYYMMDD') : 0, // 搜索用结束日期,
                    //"create_uid":sessionStorage.getItem('id'),//创建者id
                    // "bill_status": 0
                    ...
                    for(let i of arrdata){    
                        this.tablePlan.push({
                            "groupCode":i.tour_no,
                                ...
                                "destination":i.destination[0].destination_name,
                                "date":moment(i.set_out_time*1000).format('YYYYMMDD'),
                                "name":this.getCreatNameArr(i.create_uid),
                                "planID":i.id//计划id？？
                        })
                    }
        ...
        //获取全部用户姓名
        getUserName(){       
            this.$http.post(this.GLOBAL.serverSrc + "/org/api/userlist",{object:{}})
                .then(res => {                
                    let tables=res.data.objects
                window.localStorage.setItem("getCreatData", JSON.stringify(tables));
            })
            .catch(function (obj) {})
        },
        //获取产品录入人
        getCreatNameArr(id){
            let tableStr= window.localStorage.getItem("getCreatData");
            let tableArr=[];
        ...
        //选择业务类型发生变化时清空页面
        productTypeChange(){    
            this.saveType=this.ruleForm.productType;
            console.log('业务类型发生变化时savetype'+this.saveType);
            this.ruleForm.plan='';
            this.ruleForm.plan_01='';
            this.tour_id = 1;
            this.getPaymentdetails(this.tour_id,this.saveType)
        },
        // 选择账户弹窗的表格选择事件
        ...
        // 申请无收入借款中团期计划选择弹窗（搜索）
        planSelect(productTypeParams) {
            this.saveType=productTypeParams;
            this.planList(productTypeParams);
        },
        planStage(){
            this.plan_stage = '';
        ...
        routerHandle4() {        
            ...
            this.ruleForm.productType=this.saveType;
            this.getPaymentdetails(this.tour_id,this.saveType);
            this.dialogFormVisible_plan = false;
            this.plan_stage = '';
            this.plan_name = '';
        ...
        departure(item){
            ...
            console.log('供应商选中'+this.supplier_id)
        ...
        // 供应商所有关联列表信息 val代表tour_id types代表业务类型
        getPaymentdetails(val,types) {
            var that = this
            // 预付付款明细
