// 无收入借款、预付款公用的申请详情页面 引入mixins.js
<template>
 <div class="loan-management">
    <div style="text-align: right; margin:25px 20px 0 0;">
        ...
        <el-button type="success"  @click="touchPrint" plain
    ...
    <div ref="print">
      ...
      <el-row type="flex" class="row-bg" justify="space-around">
        ...
        <el-col :span="6">
          ...
          <el-col :span="15"><div class="grid-del ">{{ fundamental.createTime  | formatDateDetails  }}</div></el-col>
        </el-col>
      </el-row>
      <!-- 第一行 END -->
      <!-- 第二行 -->
      ...
      <el-row type="flex" class="row-bg" justify="space-around">
        ...
        <el-col :span="14">
          ...
          <el-col :span="18">
            <el-upload  class="upload-demo print-hidden"  name="files"  :file-list="fundamental.files"
...
export default {
  ...
  created(){  
    ...
    //获取参数
    this.acoutInfo= this.$route.query.acoutInfo;//行信息
    this.paymentType=this.$route.query.acoutInfo.paymentType;//预付款或者无收入
    this.guid=this.$route.query.acoutInfo.guid
    this.paymentID = this.$route.query.acoutInfo.paymentID
    this.status= this.$route.query.acoutInfo.checkTypeEX; //借款状态
    this.productType=this.$route.query.acoutInfo.productType;//传过来的业务类型？？
    this.getLabel();
    if(this.paymentType==1){
      this.presentRouter='无收入借款管理';
    }
    else if(this.paymentType==2){
       this.presentRouter='预付款管理';
    }
  },
  methods: {
    moment, 
    ...
    // 获取一条信息
    getLabel(){
      console.log('获取一条信息this.paymentID'+this.paymentID);
      this.$http.post(this.GLOBAL.serverSrc + '/finance/payment/api/get',{
        ...
        if(res.data.isSuccess == true){
          ...
          let getPaymentType = res.data.object.paymentType;//预付款还是借款
          this.productType= res.data.object.productType;//业务类型
          this.keepPaymentType = getPaymentType
          this.fundamental=res.data.object;
          console.log('获取一条fundamental')
          console.log(res.data.object);
          //this.tour_id = res.data.object.planID;
          this.auditResult(res.data.object.guid, getPaymentType);
          if(res.data.object.planID>0){
            this.getPaymentdetails(res.data.object.planID,this.productType);
          }else{
            console.log('进bbb')
            this.bbb(createUserCode,this.productType);
          }
        }
     })
    },
    ...
    repeal(){
      console.log('撤销::'+this.paymentType)
    ...
    // 获取相关表格信息
    getPaymentdetails(val,productTypeParams) {
      var that = this
      //相关信息
