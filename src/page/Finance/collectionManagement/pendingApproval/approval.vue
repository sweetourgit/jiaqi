<template>
  <div class="vivo" style="position:relative" id="collectionDetail">
    <el-dialog title="审批" :visible="dialogFormVisible" custom-class="city_list" :show-close="false" style="margin:-80px 0 0 0;width: 100%;">
      <div class="buttonDv">
        <el-button type="primary" icon="el-icon-remove" @click="closeAdd('cancal')" style="margin-right: 10px" plain>取消</el-button>
        <!--<el-button type="primary" @click="deleteDo" v-if="baseInfo.approved != 1">删除</el-button>-->
        <el-button icon="el-icon-circle-check" type="success" plain @click="approvalPass" :disabled="passDisabled">通过</el-button>
        <el-button icon="el-icon-circle-close" type="danger" plain @click="approvalReject" v-if="info.collectionType !== 6">驳回</el-button>
        <!-- 报销还款审批页的去认款按钮显示begin -->
        <el-button type="primary" v-if="info.collectionType == 6 && baseInfo.accountID !== 13 && isLookBtn == false && hasSubject"
          @click="recognitionDo(tableAssociated[0])">去认款</el-button>
        <el-button type="primary" v-if="info.collectionType == 6 && baseInfo.accountID !== 13 && isLookBtn == true "
          @click="recognitionDetail(tableAssociated[0])">查看</el-button>
        <!-- 报销还款审批页的去认款按钮显示end -->
        <el-button type="primary" icon="el-icon-printer" plain @click="touchPrint" plain v-if="getOrgID == 491 && info.collectionType != 6">打印本页详情</el-button>
      </div>
      ...
      <div v-if="info.collectionType !== 6">
        ...
        <div class="stepDv bottomDis">
          <el-table ref="singleTable" :data="tableDataResult" border style="width: 96%;margin: 0 auto;"
          ...
          <div class="stepDv bottomDis" v-if="info.collectionType == 2 || info.collectionType == 1">
            ...
            <el-table :data="tableAssociated" border :header-cell-style="getRowClass">
              ...
              <el-table-column prop="prop" label="操作" align="center" v-if="hasSubject">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.hasSubmitData" type="text" @click="recognitionDo(scope.row)">去认款</el-button>
      ...
      <!--审批弹窗-->
      <div>
        <el-dialog style="text-align: left;" :title="approvalTitle" :visible.sync="dialogVisibleApproval" width="30%"
          top="20vh" append-to-body>
          <div>
