      ...
      <el-table :data="tableDate" ref="multipleTable" class="table" :header-cell-style="getRowClass" border :cell-style="getCellClass" @row-click="clickRow" @selection-change="changeFun">
        ...
        <el-table-column prop="state" label="状态" align="center">
          <template slot-scope="scope">
            <div v-if="scope.row.state=='申请退款'" style="color: #7F7F7F" >{{scope.row.state}}</div>
            <div v-if="scope.row.state=='拒绝退款'" style="color: #FF4A3D" >{{scope.row.state}}</div>
            <div v-if="scope.row.state=='完成退款'" style="color: #33D174" >{{scope.row.state}}</div>
          </template>
        </el-table-column>
