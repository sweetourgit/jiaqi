...
<!-- 2020-3-31 -->
<template>
  <el-dialog  :title="isSave? '编辑报账信息':'新增报账信息' " width="650px" ref="dialogRef" :visible="state"
    ...
    <el-form label-width="120px" ref="submitForm" :model="submitForm" :rules="rules">
      <el-form-item label="供应商：" prop="supplier" ref="supplierRef">
        <el-autocomplete class="inline-input" size="small" style="width: 450px;" placeholder="输入供应商名称" v-model="submitForm.supplier"
          :fetch-suggestions="getSupplierAction" :trigger-on-focus="false" @select="selectSupplier"></el-autocomplete>
      </el-form-item>
      ...
      <el-form-item label="借款金额：" prop="arrearsPrice">
        <el-input placeholder="请输入" size="small" style="width: 450px;" v-model="submitForm.arrearsPrice"></el-input>
      </el-form-item>

      <el-form-item label="人数：" prop="peopleCount">
        <el-input placeholder="请输入" size="small" style="width: 450px;" v-model="submitForm.peopleCount"></el-input>
      </el-form-item>
    </el-form>
...
<script>
  ...
  /**
   * 这个是用于获取4D属性
   * 在写银行流水单时创建 个人使用 若项目都有需求则单独拉出来
   */
  import * as utils from '@/page/Finance/bankStatement/utils.js'
  const DATA4D = utils.getSession4D() //获取4D属性
  export default {
    data() {
    ...
    methods: {
      ...
      wakeup(expense, {
        ...
        this.expenseCache = this.isSave ? // ? this.getDraftForEdit(expense) 从草稿取值并赋予编辑表单 暂时废弃
          expense
          //用于从草稿获取添加表单信息
          :
          this.getExpenseDTO();
      ...
      saveAction() {
        ...
        this.$refs.submitForm.validate(result => {
          ...
          if (this.typed == 'draft') {
            ...
            saveDraft(that.expenseCache).then(res=>{
            })
          }
          this.handleEmit()
          this.handleClose();
