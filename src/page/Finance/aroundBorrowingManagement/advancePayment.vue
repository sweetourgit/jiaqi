        ...
        <el-row>
          ...
          <el-col :span="9">
            ...
            <el-select v-model="reimbursed_status" placeholder="请选择" class="search_input">
              ...
              <el-option key="1" label="未完整报销" value="1"></el-option>
              <el-option key="2" label="已完整报销" value="2"></el-option>
            </el-select>
          </el-col>
        </el-row>
        <el-row>
    ...
    methods: {
      ...
      exportFun(){
        ...
        if(this.tableData.length == 0){
          ...
          window.location.href = this.GLOBAL.serverSrcPhp + "/api/v1/loan/periphery-loan/exportloan?periphery_type=2&supplier_code=" + this.supplierID + "&create_uid=" + this.reimbursementPerID + "&start_time=" + start + "&end_time=" + end + "&approval_status=" + this.borrowStatus + "&account_type=" + this.accountType + "&reimbursed_status=" + this.reimbursed_status+"&USERID="+USERID;
        }
      },
      // 表格头部背景颜色
        ...
        this.$http.post(this.GLOBAL.serverSrcPhp + "/api/v1/loan/periphery-loan/listpage", {
          ...
          if (response.data.code == '200') {
            ...
            that.tableData.forEach( async function (item, index, arr) {
                   // 2020-4-20 添加审批时间字段
              item.approval_at= await getApprovalTime(item.id,2)
