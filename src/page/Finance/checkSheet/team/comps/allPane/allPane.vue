  ...
  <div class="all-pane">
    ...
    <footer>
      <div style="display: flex; justify-content: center; padding-top: 20px;">
        <el-pagination background ref="pagination" v-if="inited" :current-page.sync="pageInfo.pageIndex" :page-sizes="[2, 4, 8, 10]"
          :page-size.sync="pageInfo.pageSize" :total="pageInfo.total" @current-change="pageActionHandler" @prev-click="pageActionHandler"
          @next-click="pageActionHandler" @size-change="sizeChangeFunc" layout="total, sizes, prev, pager, next, jumper"></el-pagination>
      </div>
    </footer>
  </div>
</template>
    ...
    methods: Object.assign(
      // 常规方法
      {
        ...
        toDetailPage(row) {
          ...
          let {
            object,
            ...pageInfo
          } = this.getCheckSheetListData();
          let name = ""
          if (productType == 1) name = '报账单详情-跟团游'
          if (productType == 4) name = '报账单详情-邮轮'
          this.$router.push({
      ...
      // actions
      {
        ...
        getCheckSheetListAction() {
          ...
          object.productType = 0 //1代表跟团游 4代表游轮 0代表所有类型
          getCheckSheetList({
              object,
              pageIndex,
              pageSize
