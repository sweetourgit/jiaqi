<template>
  <div class="vivo" style="position:relative">
    <!--认收款-->
    <el-dialog title="认收款" :visible="dialogFormVisible3" width=90% @close="closeAdd">
      <div class="tableDv">
        <!--订单信息-->
        <div class="lineTitle"><i class="el-icon-info"></i>&nbsp;&nbsp;已关联&nbsp;{{totalItem}}&nbsp;项 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;总计：{{totalMoney}}元</div>
        <div class="table_trip">
          <el-table ref="multipleTable1" :data="tableData" border :header-cell-style="getRowClass">
...
<script type="text/javascript">
  ...
  export default {
    ...
    watch: {
      dialogFormVisible3: {
        handler: function () {
//          console.log('this.infoBatch',this.infoBatch);
          if(this.infoBatch === ''){
            this.closeAdd();
          }else{
            this.loadData();
...
//        console.log(item);
      ...
      loadData(){
        const that = this;
        this.$http.post(this.GLOBAL.serverSrcPhp + "/api/v1/order/external-order/batchrecorderinfo", {
          "order_sn": this.infoBatch
        }, ).then(function(response) {
          if (response.data.code == '200') {
//            console.log(response);
            that.tableData = [];
            let total = 0;
            response.data.data.forEach(function (item, index, arr) {
        ...
        this.$http.post(this.GLOBAL.serverSrcPhp + "/api/v1/order/external-order/batchreceiptlist", {
          "limit": 0
        }, ).then(function(response) {
          if (response.data.code == '200') {
            console.log(response);
