<template>
  ...
  <div @click="handleList">
    ...
    >
        ...
        <div :class="destinationInput">
          ...
          <div style="float: left">
            <el-input
              ...
              class="input_tag"
              v-model="inputTag"
              @keyup.enter.native="handleInputConfirm(inputTag)"
              @blur="handleInputConfirm(inputTag)"
      ...
      >
        <el-radio-group v-model="form.referenceTime">
          <el-radio
            v-for="(item, index) in referenceTime"
            :key="index"
            :label="item.id"
      ...
      <div style="width: 1200px; float: left">
        <div
          v-if="isDataTime"
          style="margin-left:90px;width: 565px;background: #F7F7F7; padding: 20px;margin-bottom: 20px;float: left;"
        >
          <!-- 周一 -->
          <div v-if="dateTime.one.length != 0" style="margin-top: -5px;float: left; width: 900px">
            ...
            <div v-if="!dateTime.one[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.one"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.one.length > 1 && index != dateTime.one.length - 1">;</span>
              </div>
            </div>
          ...
          <!-- 周二 -->
          <div
            v-if="dateTime.tue.length != 0"
            style="margin-top: -5px;float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周二</span>
            <div v-if="!dateTime.tue[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.tue"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.tue.length > 1 && index != dateTime.tue.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
          ...
          <!-- 周三 -->
          <div
            v-if="dateTime.wed.length != 0"
            style="margin-top: -5px;float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周三</span>
            <div v-if="!dateTime.wed[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.wed"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.wed.length > 1 && index != dateTime.wed.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
          ...
          <!-- 周四 -->
          <div
            v-if="dateTime.thur.length != 0"
            style="margin-top: -5px;float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周四</span>
            <div v-if="!dateTime.thur[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.thur"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.thur.length > 1 && index != dateTime.thur.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
          ...
          <!-- 周五 -->
          <div
            v-if="dateTime.fir.length != 0"
            style="margin-top: -5px;float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周五</span>
            <div v-if="!dateTime.fir[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.fir"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.fir.length > 1 && index != dateTime.fir.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
          ...
          <!-- 周六 -->
          <div
            v-if="dateTime.sat.length != 0"
            style="margin-top: -5px;float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周六</span>
            <div v-if="!dateTime.sat[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.sat"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.sat.length > 1 && index != dateTime.sat.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
          ...
          <!-- 周日 -->
          <div
            v-if="dateTime.sun.length != 0"
            style="margin-top: -5px; float: left;width: 900px; margin-top: 10px"
          >
            <span style="float: left">周日</span>
            <div v-if="!dateTime.sun[0].alltime" style="margin-left:84px">
              <div
                v-for="(item, index) in dateTime.sun"
                :key="index"
                style="float: left; padding-left:10px"
              >
                <span>{{item.start}}</span> -
                <span>{{item.end}}</span>
                <span v-if="dateTime.sun.length > 1 && index != dateTime.sun.length - 1">;</span>
              </div>
            </div>
            <div v-else style="margin-left:92px">
              <span>全天营业</span>
    ...
    <div class="operation">
      <el-button
        style="width: 100px; border:1px solid #3095fa;color:#3095fa;"
    ...
    >
      <OpenTime
        v-on:timeList="timeList"
        v-on:closeButton="showtime = false"
        :dateTime="dateTime"
        v-if="hackReset"
      ></OpenTime>
    </el-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        tags: [{ validator: tagRule, trigger: "blur" }],
        lat: [{ required: true, message: "请填写地理坐标", trigger: "blur" }],
        lng: [{ required: true, message: "请填写地理坐标", trigger: "blur" }],
        referenceTime: [{ required: true, message: "请选择用时" }],
        openingHours: [{ validator: openingHoursRule }]
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    imgClickShow(data) {
      this.$http
        .post("http://192.168.1.186:3024" + "/picture/api/get", {
          id: data.img_ID
        })
        .then(res => {
          this.isImgUrlShow = true;
          this.imgUrlShow =
            "http://192.168.1.186:3009/upload" + res.data.object.url;
        });
    },
    // 图片添加
    checkList(data) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          var results = queryString
            ? this.vague.filter(this.createFilter(queryString))
            : [];
          cb(results);
        });
    },
    ...
    addsave() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          let openingHours = this.dateTime.one
            .map(v => {
              if (v.alltime) {
                return {
                  theWeek: 1,
          ...
          if (this.handleEditDate == "") {
            this.$http
              .post(this.GLOBAL.serverSrc + "/scenicspot/api/insert", {
                object: {
            ...
            // 编辑操作
            this.$http
              .post(this.GLOBAL.serverSrc + "/scenicspot/api/save", {
                object: {
                  id: this.form.id,
                  name: this.form.englishName,
                  chineseName: this.form.chineseName,
