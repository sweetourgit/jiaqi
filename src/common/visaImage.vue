  ...
  <div>
    <div class="main-container">
      ...
      <div v-show="geography == 1" class="address-big">
        <div class="address-img" v-for="(img,index) in albumList" :key="index">
          <div class="marterialist-img" @click="getAlbum(img.id)">
            <img
              width="100%"
              height="100%"
              :src="picSrc+img.pictures[0].url"
              v-if="img.pictures.length>0"
            />
            <img width="100%" height="100%" :src="'../../../static/materialList/nopic.jpg'" v-else />
          </div>
          <!-- 图片介绍 -->
          <div class="introduce">
            <div v-for="(item,index) in albumtype" :key="index">
          ...
          <!-- 景点名称 -->
          <div class="address-name">{{img.name}}</div>
        </div>
        <!--分页-->
        <el-pagination
          class="pagination"
    ...
    <el-dialog
      title="相册"
      :visible.sync="getAlbumForm"
      custom-class="city_list"
      :append-to-body="true"
      width="1220px"
      @close="albumInfoClose"
      class="clearfix"
        ...
        <div class="left-img" style="overflow:auto; z-index:10000">
          <el-checkbox-group v-model="checkList">
            <div v-for="(item,index) in albumInfo.pictures" :key="index" class="img-div">
              <el-checkbox class="img-div-box" :label="item">
        ...
        <div class="right-form">
          ...
          <div class="material-message">
            ...
            <div class="album-form" v-show="picInfoShow">
              <div class="album-name1">
                名称：
                <el-input v-model="pictInfo.name" placeholder="请输入素材名称" :disabled="picDisabled"></el-input>
...
export default {
  ...
  methods: {
    ...
    albumPage(
      ...
      this.$http
        .post(this.GLOBAL.serverSrc + "/tpk/album/api/page", {
          pageIndex: pageIndex,
          pageSize: pageSize,
          object: {
            name: name,
...
.left-tree {
  ...
  box-shadow: 3px 3px 3px #ededed, 3px -3px 3px #ededed, -3px 3px 3px #ededed,
    -3px -3px 3px #ededed;
  margin-left: 1%;
  overflow: auto;
}
